...
func LogI(message string) {
	...
	Info.Printf(" %s - %s", helper.FileWithFuncAndLineNum(), message)
}

// LogE - Лог ошибок.
func LogE(message error, err string) {
	log.Printf("%s \033[1;31m[E] %s: %s\033[0m", helper.FileWithLineNum(), message, err)
	Error.Fatalf("%s - %s: %s", helper.FileWithFuncAndLineNum(), message, err)
}

// LogW - Лог предупреждений.
func LogW(message string, err string) {
	log.Printf("%s \033[1;33m[W] %s: %s\033[0m", helper.FileWithLineNum(), message, err)
	Warning.Printf("%s - %s: %s", helper.FileWithFuncAndLineNum(), message, err)
}
