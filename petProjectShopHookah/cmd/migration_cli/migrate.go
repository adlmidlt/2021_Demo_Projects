	...
	migrateToVCmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			for _, ver := range args {
				...
				internal.CheckError(msg.ErrConv, err)
				verMigr = num
			}
...
func NewMigrateInstance() (*migrate.Migrate, error) {
	...
	internal.CheckError(msg.ErrCreateInstanceDB, err)

	fileSource, err := (&file.File{}).Open("file://../../internal/migrations")
	internal.CheckError(msg.ErrFileWithMigrNotFound, err)

	m, err := migrate.NewWithInstance("file", fileSource, "shop_hookah", dbDriver)
	internal.CheckError(msg.ErrCreateNewMigrInstance, err)

	defer db.CloseDB(err)
	return m, err
}
